...
function abrirModalRenombrarProducto(nombreActual) {
    Swal.fire({
        ...
        if (res.isConfirmed) {
            ...
            mostrarCroissLoader();
            try {
                const r = await fetch('/api/stock/renombrar_producto', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({ nombre_actual: nombreActual, nombre_nuevo: res.value })
                });
                const data = await r.json();
                await esperarAnimacionMinima(tInicio, 1800);

                if (data.status === 'exito') {
                    mostrarCroissExito('Producto Renombrado', data.mensaje);
                    cargarStock(true);
                } else {
                    Swal.fire('Error', data.mensaje, 'error');
                }
            } catch (err) {
                Swal.fire('Error', 'No se pudo renombrar el producto', 'error');
            } finally {
                cerrarCroissLoaderSeguro();
            }
        }
                ...
                tooltip: {
                    ...
                    callbacks: {
                        ...
                        label: function(context) {
                            ...
                            return [`🥐 Vendidos: ${cant} un.`, `💵 Facturado: $${monto}`];
                        }
                    }
                }
            },
...
function guardarPlantillaPromo() {
    ...
    Swal.fire({ title: 'Guardado', text: 'Plantilla de WhatsApp actualizada.', icon: 'success', timer: 1500, showConfirmButton: false });
}
...
function verDetalleCliente(clienteObj) {
    ...
    if (elStats) {
        ...
        if (clienteObj.dias_sin_comprar !== undefined && clienteObj.dias_sin_comprar !== 999) {
            if (clienteObj.dias_sin_comprar < 0) txtUltimaCompra = 'Pedido Agendado';
            else if (clienteObj.dias_sin_comprar === 0) txtUltimaCompra = 'Hoy';
            else txtUltimaCompra = `Hace ${clienteObj.dias_sin_comprar} día${clienteObj.dias_sin_comprar > 1 ? 's' : ''}`;
        }
        ...
        <head>
            ...
            <style>
                * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Plus Jakarta Sans', -apple-system, sans-serif; }
                ...
                .btn-close-plaque:active { background: rgba(255, 255, 255, 0.3); }
                .story-card {
                    width: 100%;
                    max-width: 410px;
                    height: 730px;
