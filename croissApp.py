...
def calcular_costo_y_empaque_pedido(desc_producto, total_croissants):
    ...
    if desc_producto:
        ...
        for item in partes:
            ...
            # Rellenos (Se agrega Pain Au Chocolat utilizando el modelo de costo de los salados)
            if "pain au chocolat" in sabor_item or "chocolat" in sabor_item:
                c_unit += costos_dinamicos["extra_salado"]
            elif "jamon" in sabor_item or "jamón" in sabor_item or "queso" in sabor_item or "creme" in sabor_item or "crema" in sabor_item or "milano" in sabor_item:
                c_unit += costos_dinamicos["extra_salado"]
...
def stock_congelados():
    try:
        ...
        if request.method == 'POST':
            ...
            if row_masas:
                ejecutar_con_reintento(sheet_stock.update_cell, row_masas, col_stock, st_masas)
...
def stock_pop():
    try:
        ...
        if request.method == 'POST':
            ...
            if row_pop_masas:
                ejecutar_con_reintento(sheet_stock.update_cell, row_pop_masas, col_stock, st_pop_masas)
